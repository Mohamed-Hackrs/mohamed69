...
UNAPPROVED_MSG = "**نظام حماية جمثون الخاص بـ {ON}!**\n\n{UND}\n\nلديك {warn}/{twarn} تحذيرات"
if jmdB.get_key("PM_TEXT"):
    UNAPPROVED_MSG = (
        "**نظام حماية جمثون الخاص بـ {ON}!**\n\n"
...
if jmdB.get_key("PMSETTING"):
        ...
        if not keym.contains(user.id) and event.text != UND:
            ...
            if user.id in LASTMSG:
                ...
                if event.text != prevmsg:
                    if "نظام حماية جمثون" in event.text or "**نظام حماية جمثون" in event.text:
                        return
    ...
    @mohamed_cmd(pattern="(تشغيل|ايقاف|مسح) الارشفة$", fullsudo=True)
    async def _(e):
    ...
    @mohamed_cmd(pattern="(س|سماح)(?: |$)", fullsudo=True)
    async def approvepm(apprvpm):
        ...
        if user.id in DEVLIST:
            return await apprvpm.eor(
                "**⌔∮ هذا هو مطور سورس جمثون هو بالأصل مسموح له بالدردشة  !**",
            )
    ...
    @mohamed_cmd(pattern="(ر|رفض)(?: |$)", fullsudo=True)
...
@mohamed_cmd(pattern="بلوك( (.*)|$)", fullsudo=True)
...
@mohamed_cmd(pattern="الغاء البلوك( (.*)|$)", fullsudo=True)
...
@mohamed_cmd(pattern="قائمة المسموحين$", owner=True)
...
async def in_pm_ans(event):
    ...
    try:
        ...
        msg_ = "**⌔∮ نظام حماية جمثون الخاص بـ {OWNER_NAME}**"
    wrns = f"{warns}/{WARNS}"
    buttons = [
        [
    ...
    await event.answer(res, switch_pm="• Jmthon •", switch_pm_param="start")
