...
from .. import JmdB, jmubot, mohamed_cmd



@mohamed_cmd(pattern="منع( (.*)|$)", admins_only=True)
async def af(e):
...
@mohamed_cmd(pattern="الغاء منع( (.*)|$)", admins_only=True)
async def ref(e):
    moh = e.pattern_match.group(1).strip()
    chat = e.chat_id
    if not moh:
...
@mohamed_cmd(pattern="قائمة المنع$", admins_only=True)
async def lsrf(e):
