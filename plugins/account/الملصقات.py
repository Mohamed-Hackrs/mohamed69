...
from jmthon.helper.tools import resize_photo_sticker

from .. import LOGS, tgbot, fetch, jmdB, mohamed_cmd, inline_mention, create_quotly
...
async def pack_kangish(_):
    ...
    _packname = cmdtext or f"Jmthon Stickers of {_.sender_id}"
    typee = None
    if not local:
...
@mohamed_cmd(pattern="ستيكر", manager=True)
...
@mohamed_cmd(pattern="الحزم", manager=True)
